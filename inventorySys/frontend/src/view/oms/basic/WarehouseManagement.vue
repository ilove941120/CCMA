...
//#region 刪除資料
const DeleteData = async (WarehouseId) => {
    const obj =reactive({WarehouseId:WarehouseId})
    try{
        const result = (await DeleteWarehouse(obj))
...
const LoadData = async () => {
    try{
      const result = (await GetWarehouse(pageObj)).data
...
<template>
    ...
    <div class="content" v-if="editShow == false">
        <table>
            ...
            <tr class="tableItem" v-for="(item, index) in tableData" :key="item.WarehouseId" @mouseover="hoverRow(true, item.WarehouseId)" @mouseout="hoverRow(false, item.WarehouseId)">
                <td :style="{ backgroundColor: highlightedRow === item.WarehouseId ? '#C8EBFA' : '' }">
                    <div class="coulumName">#</div>
                    <div class="coulumValue">{{ pageObj.Index > 0 ? index + pageObj.Index + 1 : index + 1 }}</div>
                </td>
                <td :style="{ backgroundColor: highlightedRow === item.WarehouseId ? '#C8EBFA' : '' }">
                    <div class="coulumName">庫別代號</div>
                    <div class="coulumValue">{{ item.WarehouseNo }}</div>
                </td>
                <td :style="{ backgroundColor: highlightedRow === item.WarehouseId ? '#C8EBFA' : '' }">
                    <div class="coulumName">庫別名稱</div>
                    <div class="coulumValue">{{ item.WarehouseName }}</div>
                </td>
                <td :style="{ backgroundColor: highlightedRow === item.WarehouseId ? '#C8EBFA' : '' }">
                    <div class="coulumName">庫別描述</div>
                    <div class="coulumValue">{{item.WarehouseDesc}}</div>
                </td>
                <td :style="{ backgroundColor: highlightedRow === item.WarehouseId ? '#C8EBFA' : '' }">
                    ...
                    <div class="coulumValue">
                        ...
                        <button @click="DeleteData(item.WarehouseId)">刪除</button>
                    </div>
                </td>
            </tr>
        </table>
